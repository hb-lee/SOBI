...
int matrix_mul_notran(Mat * src1, Mat * src2, Mat * dst)
{
    ...
    for (i = 0; i < 128; i++)
    {
        ...
        for (j = 0; j < 128; j++)
        {
            A[i * 128 + j] = src1->element[i][j];
            B[i * 128 + j] = src2->element[i][j];
        }
    }
    double alpha = 1.0;
    double beta = 0.0;
...
void sobi_reduce(Mat * TV, Mat * B_mult)
{
    ...
    for (i = 0; i < 128; i++)
    {
        ...
        //W_scaled_(&(W_scaled->element[i][0]), &(W_unscaled->element[i][0]), &(scaling_factor[i]), &W_unscaled->col);
        //for (j = 0; j < W_unscaled->col; j++)
        //{
